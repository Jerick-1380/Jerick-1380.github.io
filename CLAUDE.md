...
- `main.css` and `main.js` are linked with a `?v=<timestamp>` cache-buster. **After editing either file, bump the version everywhere** — there is no build step to fingerprint assets, and GitHub Pages serves them with `max-age=600`, so without a bump the change looks like it silently failed in any browser holding the old copy. Use a minute-resolution stamp, not a date: two edits on the same day must not produce the same string. One command: `V=$(date +%Y%m%d%H%M); grep -rl 'main\.css?v=' --include=*.html . | xargs sed -i '' -E "s#(main\.(css|js))\?v=[0-9]+#\1?v=$V#g"` — then do the same in `bin/build_articles.py` and regenerate.
